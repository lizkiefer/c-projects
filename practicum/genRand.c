/*
*genRand.c / Practicum I / Program in C
*
...
{
    seed = (multiplier * inSeed + increment++) % modulus;
    if (seed < 0)
    {
        seed = seed * -1;
    }
...
int genLru(int messageCacheSize, int *messageCacheCounters)
{
    int storeIndex = 0;
    int lruMin = -1;
    for (int i = 0; i < messageCacheSize; i++)
    {
        if (messageCacheCounters[i] < lruMin || lruMin < 0)
        {
            lruMin = messageCacheCounters[i];
            storeIndex = i;
        }
    }
    return storeIndex;
